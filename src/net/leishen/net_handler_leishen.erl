...
handle(Bin, #handler_state{debug_pid = Pid} = HState) ->
    ...
    Packets = net_pack_leishen:unpacks(Bin),
...
handle_2(MsgID, Content, #handler_state{pid = ?undefined} = HState)
        when MsgID =:= 'C0000001' orelse
            MsgID =:= 'C0000015' orelse
            MsgID =:= 'C0000018' ->
    handle_local('C0000001', Content, HState);
...
handle_local(MsgID, Content, HState) ->
    try handle_login:handle_msg(Content, HState) of
